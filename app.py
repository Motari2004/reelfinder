...
class ReelData:
    ...
    def to_dict(self):
        return {
            ...
            "full_caption": str(self.full_caption) if self.full_caption else str(self.caption),
...
class UniversalReelsFinder:
    ...
    async def search(self, query: str, max_results: int = 50, search_type: str = "auto") -> List[ReelData]:
        """Enhanced search with full caption capture"""
        reels = []
        seen_shortcodes = set()
        ...
        try:
            ...
            for link_data in links:
                ...
                caption_text = link_data.get('caption', '')
    ...
    def _clean_caption(self, text: str) -> str:
        """Clean and extract the actual caption from text"""
        if not text:
            return 'No caption'
        ...
        if len(text) > 300 or 'www.instagram.com' in text:
            ...
            meaningful = []
            for sentence in sentences:
                if len(sentence) > 20 and 'instagram' not in sentence.lower() and 'http' not in sentence.lower():
                    meaningful.append(sentence.strip())
            if meaningful:
